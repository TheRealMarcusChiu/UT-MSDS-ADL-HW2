...
#             batch_first=True
#         )
#         self.projection_out = torch.nn.Linear(d_latent, n_tokens)
#
#     def forward(self, x: torch.Tensor) -> tuple[torch.Tensor, dict[str, torch.Tensor]]:
#         if len(x.shape) == 4:
#             x = bsq.load().encode_index(x.float() / 255.0 - 0.5)
#         if len(x.shape) == 2:
#             x = x.unsqueeze(0)
#         B, h, w = x.shape
...
#         output = self.projection_out(transformed)
#         return output.reshape(B, h, w, -1), {}
#
#     def generate(self, B: int = 1, h: int = 30, w: int = 20, device=None) -> torch.Tensor:
#         total = h * w
#         zeros = torch.zeros((B, total), dtype=torch.long, device=device)
#         for i in range(total):
#             embedded = self.embedding(torch.nn.functional.pad(zeros[:, :i+1], (0, total - i-1), value=0))
#             transformed = self.transformer(embedded, src_mask=torch.nn.Transformer.generate_square_subsequent_mask(total).to(device))
#             zeros[:, i] = torch.multinomial(torch.nn.functional.softmax(self.projection_out(transformed[:, i, :]), dim=-1), 1).squeeze(-1)
#         return zeros.reshape(B, h, w)
